...
このプロジェクトは `uv` で Python バージョン、依存パッケージ、lock file を管理します。

clone 後は、プロジェクトルートで以下を実行します。

```sh
git clone git@github.com:nightSky98z/NLP.git
cd NLP
uv sync
```

`uv sync` は `pyproject.toml` と `uv.lock` を読み、プロジェクト用の仮想環境へ依存パッケージを同期します。通常は `pip install ...` を直接使わず、依存関係を追加する場合は `uv add` を使います。
...
他の環境や CI で lock file 通りに同期できるか確認する場合:

```sh
uv sync --locked
```

HINT: `pip install ...` だけで導入すると `pyproject.toml` と `uv.lock` に依存が記録されず、別環境で再現できなくなります。一時検証以外では `uv add` を使ってください。
...
その embedding を `LogisticRegression` に渡して、行動カテゴリまたは対象タイプを選びます。これにより、学習データが少なくても `倒す`、`斬る`、`打倒` のような表現を combat に寄せやすくしています。
...
そのため、学習データは大量でなくてもよいですが、各カテゴリの代表例は必要です。例えば combat には `倒す` だけでなく、`斬る`、`打倒`、`攻撃する` のような言い換えを少数入れておくと境界が安定します。
